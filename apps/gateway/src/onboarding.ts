...
router.post('/', async (req: any, res, next) => {
  try {
    ...
    res.status(201).json({
      tenant,
      note: 'Tenant created and assigned to the requested app. Your current session token is NOT yet updated with this tenant — depending on your identity provider, you may need to sign in again before tenant-scoped routes will work.',
    });
  } catch (err) { next(err); }
});
