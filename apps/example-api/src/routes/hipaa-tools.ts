/**
 * apps/example-api/src/routes/hipaa-tools.ts
 * HIPAA Tools — PHI access logging.
 *
 * NOTE: no DELETE — access logs are compliance records and must not be
 * removable via this API (that would defeat their purpose).
 */
...
const ListLogsQuerySchema = PaginationQuerySchema.extend({
  patientRef: z.string().min(1).max(255),
});
...
router.get('/items',
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      await auditEmit({ tenantId, actorId: userId, actorType: 'user', action: 'data.read', outcome: 'success', resource: 'phi_access_log', description: `Listed PHI access logs for ${query.patientRef} (page cursor: ${query.cursor || 'start'})` });
...
router.post('/items',
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const rows = await withTenantQuery<{ id: string; accessor_id: string; access_reason: string; accessed_at: string }>(
        `INSERT INTO phi_access_logs (tenant_id, accessor_id, patient_ref, access_reason)
         VALUES ($1, $2, $3, $4)
         RETURNING id, accessor_id, access_reason, accessed_at`,
        [tenantId, userId, input.patientRef, input.accessReason],
        tenantId,
      );

      const log = rows[0];

      await auditEmit({ tenantId, actorId: userId, actorType: 'user', action: 'data.created', outcome: 'success', resource: 'phi_access_log', resourceId: log.id, description: `Logged PHI access to ${input.patientRef}: ${input.accessReason}` });
...
export { router as hipaaToolsRouter };
