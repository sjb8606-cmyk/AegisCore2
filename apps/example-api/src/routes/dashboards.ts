/**
 * apps/example-api/src/routes/dashboards.ts
...
const DashboardIdParamSchema = z.object({ id: z.string().uuid() });
...
router.get('/items',
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const rows = await withTenantQuery<{ id: string; name: string; layout: Record<string, unknown>; created_at: string; updated_at: string }>(
      ...
      await auditEmit({ tenantId, actorId: userId, actorType: 'user', action: 'data.read', outcome: 'success', resource: 'dashboard', description: `Listed dashboards (page cursor: ${query.cursor || 'start'})` });
...
router.post('/items',
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      const input = req.body as z.infer<typeof CreateDashboardSchema>;
      const tenantId = req.auth!.tenantId;
      const userId = req.auth!.sub;

      const rows = await withTenantQuery<{ id: string; name: string; layout: Record<string, unknown>; created_at: string; updated_at: string }>(
        `INSERT INTO dashboards (tenant_id, user_id, name, layout)
         VALUES ($1, $2, $3, $4::jsonb)
         RETURNING id, name, layout, created_at, updated_at`,
        [tenantId, userId, input.name, JSON.stringify(input.layout)],
        tenantId,
      );

      const dashboard = rows[0];

      await auditEmit({ tenantId, actorId: userId, actorType: 'user', action: 'data.created', outcome: 'success', resource: 'dashboard', resourceId: dashboard.id, description: `Created dashboard '${input.name}'` });
      await recordUsage({ tenantId, actorId: userId, eventType: 'api_call', quantity: 1, idempotencyKey: `api:${req.method}:${req.path}:${dashboard.id}` });

      return created(res, dashboard);
    } catch (err) { next(err); }
  }
);
...
router.delete('/items/:id',
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      const { id } = DashboardIdParamSchema.parse({ id: req.params.id });
      const tenantId = req.auth!.tenantId;
      const userId = req.auth!.sub;
      ...
      if (rows.length === 0) {
        return next(new AppError('Dashboard not found', ErrorCode.NOT_FOUND));
      }

      await auditEmit({ tenantId, actorId: userId, actorType: 'user', action: 'data.deleted', outcome: 'success', resource: 'dashboard', resourceId: id, description: `Removed dashboard ${id}` });
      await recordUsage({ tenantId, actorId: userId, eventType: 'api_call', quantity: 1, idempotencyKey: `api:${req.method}:${req.path}:${Date.now()}` });
