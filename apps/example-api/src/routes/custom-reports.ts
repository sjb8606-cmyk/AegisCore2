/**
 * apps/example-api/src/routes/custom-reports.ts
...
const CreateReportSchema = z.object({
  ...
  queryDefinition: z.record(z.any()).default({}),
});
...
router.get('/items',
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const rows = await withTenantQuery<{ id: string; name: string; query_definition: Record<string, unknown>; created_at: string; updated_at: string }>(
        `SELECT id, name, query_definition, created_at, updated_at FROM custom_reports
         WHERE user_id = $1
      ...
      await auditEmit({ tenantId, actorId: userId, actorType: 'user', action: 'data.read', outcome: 'success', resource: 'custom_report', description: `Listed custom reports (page cursor: ${query.cursor || 'start'})` });
...
router.post('/items',
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      const input = req.body as z.infer<typeof CreateReportSchema>;
      const tenantId = req.auth!.tenantId;
      const userId = req.auth!.sub;

      const rows = await withTenantQuery<{ id: string; name: string; query_definition: Record<string, unknown>; created_at: string; updated_at: string }>(
        `INSERT INTO custom_reports (tenant_id, user_id, name, query_definition)
         VALUES ($1, $2, $3, $4::jsonb)
         RETURNING id, name, query_definition, created_at, updated_at`,
        [tenantId, userId, input.name, JSON.stringify(input.queryDefinition)],
        tenantId,
      );

      const report = rows[0];

      await auditEmit({ tenantId, actorId: userId, actorType: 'user', action: 'data.created', outcome: 'success', resource: 'custom_report', resourceId: report.id, description: `Created report '${input.name}'` });
...
router.delete('/items/:id',
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      const { id } = ReportIdParamSchema.parse({ id: req.params.id });
      const tenantId = req.auth!.tenantId;
      const userId = req.auth!.sub;
      ...
      if (rows.length === 0) {
        return next(new AppError('Report not found', ErrorCode.NOT_FOUND));
      }

      await auditEmit({ tenantId, actorId: userId, actorType: 'user', action: 'data.deleted', outcome: 'success', resource: 'custom_report', resourceId: id, description: `Removed report ${id}` });
      await recordUsage({ tenantId, actorId: userId, eventType: 'api_call', quantity: 1, idempotencyKey: `api:${req.method}:${req.path}:${Date.now()}` });
