...
describe('ShipmentIntakeService.logShipment', () => {
  it('verifies the species is real before logging, then records a real shipment', async () => {
    (withTenantQuery as any).mockResolvedValue([{ id: SHIPMENT_ID, weight_kg: 850.5 }]);

    const result = await ShipmentIntakeService.logShipment(TENANT_ID, USER_ID, validInput);
  ...
  it('propagates NOT_FOUND when the species does not genuinely exist', async () => {
    ...
    await expect(
      ShipmentIntakeService.logShipment(TENANT_ID, USER_ID, validInput)
  ...
  it('throws BAD_REQUEST for an invalid userId', async () => {
    await expect(
      ShipmentIntakeService.logShipment(TENANT_ID, 'not-a-uuid', validInput)
...
describe('ShipmentIntakeService.listShipments', () => {
  it('applies speciesId and date range filters to the real query', async () => {
    ...
    expect(call[0]).toContain('catch_date >=');
    expect(call[0]).toContain('catch_date <=');
...
describe('ShipmentIntakeService.getTotalWeightForSpecies', () => {
  it('returns the real summed weight', async () => {
  ...
  it('returns 0 when there are no matching shipments (COALESCE, not undefined/NaN)', async () => {
